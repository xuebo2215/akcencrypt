...
static void akc_sm3_process( akc_sm3_context *ctx, unsigned char data[64] )
{
...
void akc_sm3_update( akc_sm3_context *ctx, unsigned char *input, int ilen )
{
    ...
    if( left && ilen >= fill )
    {
        memcpy( (void *) (ctx->buffer + left),
                (void *) input, fill );
        akc_sm3_process( ctx, ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
...
static const unsigned char akc_sm3_padding[64] =
{
...
void akc_sm3_finish( akc_sm3_context *ctx, unsigned char output[32] )
{
    ...
    akc_sm3_update( ctx, (unsigned char *) akc_sm3_padding, padn );
    akc_sm3_update( ctx, msglen, 8 );
           ...
           unsigned char output[32] )
{
    ...
    akc_sm3_starts( &ctx );
    akc_sm3_update( &ctx, input, ilen );
    akc_sm3_finish( &ctx, output );

    memset( &ctx, 0, sizeof( akc_sm3_context ) );
}
...
void akc_sm3_hmac_starts( akc_sm3_context *ctx, unsigned char *key, int keylen )
{
    ...
    akc_sm3_starts( ctx);
    akc_sm3_update( ctx, ctx->ipad, 64 );
...
void akc_sm3_hmac_finish( akc_sm3_context *ctx, unsigned char output[32] )
{
    ...
    akc_sm3_finish( ctx, tmpbuf );
    akc_sm3_starts( ctx );
    akc_sm3_update( ctx, ctx->opad, 64 );
    akc_sm3_update( ctx, tmpbuf, hlen );
    akc_sm3_finish( ctx, output );
                ...
                unsigned char output[32] )
{
    ...
    akc_sm3_hmac_starts( &ctx, key, keylen);
    akc_sm3_hmac_update( &ctx, input, ilen );
    akc_sm3_hmac_finish( &ctx, output );
